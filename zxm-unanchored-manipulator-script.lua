-- AETHER MANIPULATOR v2.1
-- Fixed toggle visibility, starts open, "ZUMS" toggle button
...
local function createMainGUI()
		...
		btn.MouseButton1Click:Connect(function()
			...
			for name, b in pairs(tabButtons) do
				if name == tabName then
					tween(b, {BackgroundColor3 = ACCENT, BackgroundTransparency = 0, TextColor3 = Color3.new(1,1,1)}, 0.2)
				else
					tween(b, {BackgroundColor3 = Color3.new(0,0,0), BackgroundTransparency = 0.6, TextColor3 = TEXT_SECONDARY}, 0.2)
				end
			end
			switchTab(tabName)
		end)
	...
	addToggle(styleFrame, "Glow Pulse", 3, false, function(v)
		glowParts = v
	end)
	...
	addToggle(physFrame, "Auto-Scan Nearby", 11, false, function(v)
		-- Could implement auto-scan loop here if desired
	end)
...
Table
Feature	Description
Floating Toggle Orb	A pulsing, glowing button at the bottom-right that opens the main UI. Minimizing returns to this orb.
4 Clean Tabs	SHAPES (16 formations), STYLE (rainbow, neon, colors), PHYSICS (radius, strength, spin), SYSTEM (status, release, destroy)
Aesthetic "Aether" Theme	Deep midnight glass with purple/cyan accents, gradient top bar, rounded corners, subtle strokes
More Shapes	Added Grid, Tornado, and Flower to the original 12
Visual Effects	Rainbow cycle, forced neon material, solid color picker palette, glow pulse toggle
Better Feedback	Live status label, hover animations on all buttons, active tab highlighting
No Ownership Hijacking	Still only uses natural physics + BodyPosition/BodyGyro
The UI now feels like a modern tool rather than a cluttered panel. Let me know if you want keybinds (e.g., press R to release, Q to toggle UI) or sound effects on button clicks!
Seems like when the toggle button was pressed, it didn't show or there's nothing appears after it. Please fix this bug and also make the script when executed to be in showed state not hidden state (toggled); for the toggle button, change it to text for the icon instead of shape icon or something like that (text: "ZUMS")
Edit
Copy
Share

Kimi
I see the issue — the toggle button's Enabled property was being set to false which hides the entire ScreenGui, but the main GUI creation wasn't properly handling the visibility state. Also, the initial state should show the main GUI immediately. Let me fix this and update the toggle icon to "ZUMS" text.
lua
Copy
-- AETHER MANIPULATOR v2.1
-- Fixed toggle visibility, starts open, "ZUMS" toggle button
...
-- ==================== AESTHETIC UI SYSTEM ====================
local mainGui, toggleBtn, tabContents = nil, nil, {}
...
local function createToggleButton()
	local pg = player:WaitForChild("PlayerGui")
	local old = pg:FindFirstChild("AetherToggle")
	...
	btn.Position = UDim2.new(1, -64, 1, -72)
	...
	shadow.BackgroundColor3 = ACCENT
	...
	task.spawn(function()
		while sg.Parent do
			tween(shadow, {Size = UDim2.new(1, 12, 1, 12), Position = UDim2.fromOffset(-6, -6), BackgroundTransparency = 0.95}, 1)
			task.wait(1)
			tween(shadow, {Size = UDim2.new(1, 6, 1, 6), Position = UDim2.fromOffset(-3, -3), BackgroundTransparency = 0.85}, 1)
			task.wait(1)
		end
	end)
	
	btn.MouseEnter:Connect(function() tween(btn, {BackgroundColor3 = ACCENT_GLOW}, 0.15) end)
	btn.MouseLeave:Connect(function() tween(btn, {BackgroundColor3 = ACCENT}, 0.15) end)
	btn.MouseButton1Click:Connect(function()
		sg.Enabled = false
		createMainGUI()
	end)
...
local function switchTab(tabName)
	for name, frame in pairs(tabContents) do
		if name == tabName then
			frame.Visible = true
			tween(frame, {Position = UDim2.fromScale(0, 0), BackgroundTransparency = 1}, 0.2)
		else
			frame.Visible = false
		end
	end
...
local function createMainGUI()
	...
	-- Backdrop blur effect (simulated with frame)
	local backdrop = Instance.new("Frame")
	backdrop.Size = UDim2.new(1, 0, 1, 0)
	backdrop.BackgroundColor3 = Color3.new(0, 0, 0)
	backdrop.BackgroundTransparency = 0.6
	...
	minBtn.MouseButton1Click:Connect(function()
		sg:Destroy()
		if toggleBtn then toggleBtn.Enabled = true else createToggleButton() end
	end)
	...
	titleArea.InputBegan:Connect(function(inp)
		if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
			dragging = true; dragStart = Vector2.new(inp.Position.X, inp.Position.Y); startPos = panel.Position
		end
	end)
	UserInputService.InputChanged:Connect(function(inp)
		if dragging and (inp.UserInputType == Enum.UserInputType.MouseMovement or inp.UserInputType == Enum.UserInputType.Touch) then
...
-- ==================== INIT ====================
-- Start with main GUI visible, no toggle button
createMainGUI()